    ...
    public static function getParam($name, $type = 'string', $default = '', $base64 = false)
    {
        ...
        if (isset($_POST[$name])) {
            if (in_array($type, array('int', 'integer'))) {
                ...
                // false = 5 characters
                $tmp = substr(filter_input(INPUT_POST, $name, FILTER_SANITIZE_FULL_SPECIAL_CHARS), 0, 5);
            ...
            if (isset($_GET[$name])) {
                if (in_array($type, array('int', 'integer'))) {
                    ...
                    // false = 5 characters
                    $tmp = substr(filter_input(INPUT_GET, $name, FILTER_SANITIZE_FULL_SPECIAL_CHARS), 0, 5);
                    $return = (in_array(strtolower($tmp), array('on', 'true'))) ? true : false;
                } elseif ($type == 'string') {
        ...
        function initSort() {
            ...
            $("#tblFolders").tablesorter({
                ...
                headers: {
                    ...
                    1: {
                        sorter: "digit"
                    }, // Folder size in Ko, MB, ...
                    2: {
                        sorter: "digit"
                    } // Folder size in integer
                },
                ignoreCase: true,
                ...
                sortList: [
                    [2, 1]
                ] // Sort by default on the folder size, descending
            }); // $("#tblFolders")
            ...
            $("#tblExtensions").tablesorter({
                ...
                sortList: [
                    [3, 1]
                ] // Sort by default on the total size size, descending
            }); // $("#tblExtensions")
